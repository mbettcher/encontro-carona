...
public class TioCaronaEventoService {
    ...
    public List<TioCaronaEventoResponse> listar(Long eventoId) {
        return repository.findByEventoIdOrderByPessoaNome(eventoId)
                ...
                .map(tio -> TioCaronaEventoResponse.from(
                        tio,
                        operacaoRepository
                                .findFirstByTioCaronaEventoIdOrderByOcorridoEmDesc(
                                        tio.getId()
                                )
                                .orElse(null),
                        credencialOperacionalService
                                .buscarCredencialTioCarona(
                                        eventoId,
                                        tio.getId()
                                )
                                .orElse(null)
    ...
    public List<TioCaronaEventoOperacaoResponse> listarOperacoes(
        ...
        return operacaoRepository
                .findByTioCaronaEventoIdOrderByOcorridoEmDesc(
                        tioCaronaEventoId
                )
                .stream()
    ...
    public TioCaronaEventoResponse adicionar(
            Long eventoId,
            TioCaronaEventoRequest request
    ) {
        if (repository.existsByEventoIdAndPessoaId(
            ...
            throw new BusinessException(
                    "Esta pessoa já está cadastrada como tio carona neste evento."
            );
        }
    ...
    public TioCaronaEventoResponse registrarCheckinPorCodigo(
        ...
        var tioCarona = resolverTioPorCodigoOuCredencial(
                eventoId,
                codigoIdentificacao
        );
    ...
    public TioCaronaEventoResponse registrarCheckinManual(
        ...
        var tioCarona = buscarPorIdEvento(
                eventoId,
                tioCaronaEventoId
        );

        validarCredencialAtivaParaOperacaoManual(
                eventoId,
                tioCaronaEventoId
        );
        ...
        return registrarOperacao(
                tioCarona,
                TipoOperacaoTioCarona.CHECKIN,
                OrigemOperacaoTioCarona.MANUAL,
                null
        );
    }
    ...
    /**
     * Exclui somente o vínculo da pessoa como tio carona deste evento.
     * <p>
     * O cadastro da Pessoa permanece intacto e poderá ser usado em outros
     * eventos ou em outras funções da aplicação.
     */
    @Transactional
    public void excluir(
            Long eventoId,
        ...
        validarTioCaronaPodeSerExcluido(
                eventoId,
                tioCaronaEventoId,
                tioCarona
        );
    ...
    private void validarTioCaronaPodeSerExcluido(
        ...
        if (duplaTioCaronaRepository.existsTioEmQualquerDupla(
                eventoId,
                tioCaronaEventoId
        )) {
            throw new BusinessException(
                    "Não é possível remover " +
                            tioCarona.getPessoa().getNome() +
                            " deste evento porque o tio carona pertence a uma dupla. " +
                            "Exclua a dupla antes de remover o tio carona do evento."
            );
        }

        if (operacaoRepository.existsByEventoIdAndTioCaronaEventoId(
                eventoId,
                tioCaronaEventoId
        )) {
            throw new BusinessException(
                    "Não é possível remover " +
                            tioCarona.getPessoa().getNome() +
                            " deste evento porque já existem operações de check-in " +
                            "ou checkout registradas. O histórico precisa ser preservado."
            );
        }

        if (credencialEventoRepository
                .existsByEventoIdAndTioCaronaEventoId(
                        eventoId,
                        tioCaronaEventoId
                )) {
            throw new BusinessException(
                    "Não é possível remover " +
                            tioCarona.getPessoa().getNome() +
                            " deste evento porque já existe uma credencial emitida. " +
                            "O histórico da credencial precisa ser preservado."
            );
        }
    }
    ...
    private TioCaronaEventoResponse registrarOperacao(
        ...
        var operacao = operacaoRepository.save(
                new TioCaronaEventoOperacao(
                        tioCarona.getEvento(),
                        tioCarona,
                        tipo,
                        origem,
    ...
    private void validarSequencia(
        ...
        if (ultimaOperacao == null) {
            if (tipo == TipoOperacaoTioCarona.CHECKOUT) {
                throw new BusinessException(
                        "Não é possível realizar checkout sem check-in."
                );
            }
        ...
        if (
                ultimaOperacao.getTipo() == TipoOperacaoTioCarona.CHECKIN
                        && tipo == TipoOperacaoTioCarona.CHECKIN
        ) {
            throw new BusinessException(
                    "Já existe check-in em aberto para este tio carona."
            );
        }

        if (
                ultimaOperacao.getTipo() == TipoOperacaoTioCarona.CHECKOUT
                        && tipo == TipoOperacaoTioCarona.CHECKOUT
        ) {
            throw new BusinessException(
                    "Já existe checkout como última operação deste tio carona."
            );
        }
    }
    ...
    private void validarAtivo(TioCaronaEvento tioCarona) {
        if (tioCarona.getStatus() != TioCaronaStatus.ATIVO) {
            throw new BusinessException(
                    "Tio carona inativo não pode realizar operação."
            );
        }
    }

    private void validarCredencialAtivaParaOperacaoManual(
            Long eventoId,
            Long tioCaronaEventoId
    ) {
        var credencial = credencialOperacionalService
                ...
                .orElseThrow(() ->
                        new BusinessException(
                                "Não é possível registrar operação manual: " +
                                        "tio carona sem credencial ativa."
                        )
                );
        ...
        if (credencial.getStatus() != StatusCredencial.ATIVA) {
            throw new BusinessException(
                    "Não é possível registrar operação manual: " +
                            "a credencial do tio carona não está ativa."
            );
        }
    }

    private TioCaronaEvento buscarPorIdEvento(
            Long eventoId,
            Long tioCaronaEventoId
    ) {
        return repository
    ...
    private TioCaronaEvento resolverTioPorCodigoOuCredencial(
            Long eventoId,
            String codigoIdentificacao
    ) {
        var codigo = normalizarCodigoIdentificacao(
                codigoIdentificacao
        );

        if (credencialOperacionalService.existeCredencialComCodigo(
                codigo
        )) {
            return credencialOperacionalService
                    .resolverTioCaronaPorCredencial(
                            eventoId,
                            codigo
                    );
        }
        ...
        return repository
                .findByEventoIdAndCodigoIdentificacao(
                        eventoId,
                        codigo
                )
                .orElseThrow(() ->
                        new ResourceNotFoundException(
                                "Tio carona não encontrado para o código informado."
                        )
                );
    }

    private String normalizarCodigoIdentificacao(
            String codigoIdentificacao
    ) {
        if (
                codigoIdentificacao == null
                        || codigoIdentificacao.isBlank()
        ) {
            throw new BusinessException(
                    "Código de identificação é obrigatório."
            );
        }
