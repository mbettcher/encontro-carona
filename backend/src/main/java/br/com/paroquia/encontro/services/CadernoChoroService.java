...
public class CadernoChoroService {
    ...
    public CadernoChoroGeracaoResponse gerar(Long eventoId) {
        ...
        var vinculosAtivos = sobrinhoDuplaRepository.findByEventoIdAndStatusOrderByDuplaCodigoAscSobrinhoNomeAsc(
                eventoId,
                VinculoStatus.ATIVO
        );
        ...
        for (var vinculo : vinculosAtivos) {
            ...
            var caderno = repository.save(new CadernoChoro(
                    evento,
                    vinculo.getDupla(),
                    sobrinho
            ));

            historicoRepository.save(new CadernoChoroHistorico(
                    caderno,
                    StatusCadernoChoro.PENDENTE,
                    "Caderno gerado automaticamente a partir do vínculo ativo com a dupla."
            ));
    ...
    public List<CadernoChoroResponse> listarPorDupla(Long eventoId, Long duplaId) {
        ...
        return repository.findByEventoIdAndDuplaIdOrderBySobrinhoNomeAsc(eventoId, duplaId)
                .stream()
                .map(CadernoChoroResponse::from)
                .toList();
    }
    ...
    public List<CadernoChoroHistoricoResponse> listarHistorico(Long eventoId, Long cadernoId) {
        ...
        return historicoRepository.findByCadernoIdOrderByOcorridoEmDesc(cadernoId)
                .stream()
    ...
    public List<CadernoChoroResponse> entregarADupla(Long eventoId, Long duplaId, String observacao) {
        ...
        var cadernos = repository.findByEventoIdAndDuplaIdAndStatusIn(
                eventoId,
                duplaId,
        ...
        if (cadernos.isEmpty()) {
            throw new BusinessException("Não há cadernos pendentes para entregar a esta dupla.");
        }
    ...
    public List<CadernoChoroResponse> receberDaDupla(Long eventoId, Long duplaId, String observacao) {
        ...
        if (cadernos.isEmpty()) {
            throw new BusinessException("Não há cadernos entregues à dupla para receber de volta.");
        }
    ...
    private CadernoChoro buscarCaderno(Long eventoId, Long cadernoId) {
        return repository.findByIdAndEventoId(cadernoId, eventoId)
                .orElseThrow(() -> new ResourceNotFoundException("Caderno do Choro não encontrado neste evento."));
    }
