    ...
    <div class="card shadow-sm border-0">
      <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
          <thead class="table-light"><tr><th>nome</th><th>telefone</th><th>email</th><th>tipo</th></tr></thead>
          <tbody>
            <tr *ngFor="let item of itens()"><td>{{ item.nome }}</td><td>{{ item.telefone }}</td><td>{{ item.email }}</td><td>{{ item.tipo }}</td></tr>
...
export class PessoaListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.listarPessoas().subscribe({
