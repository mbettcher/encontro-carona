...
interface CredencialCrachaPrint {
...
@Component({
    selector: 'app-evento-cracha-print',
    standalone: true,
    imports: [
        RouterLink,
        ButtonModule,
...
export class EventoCrachaPrintComponent implements OnInit, OnDestroy {
    ...
    readonly descricao = computed(() => {
        ...
        switch (this.tipo()) {
            ...
            default:
                return 'Impressão em lote dos crachás ativos do evento.';
        }
    });

    ngOnInit(): void {
        document.body.classList.add('cracha-print-mode');
    ...
    ngOnDestroy(): void {
        document.body.classList.remove('cracha-print-mode');
        window.removeEventListener('afterprint', this.aoFinalizarImpressao);
    }

    imprimir(): void {
        if (this.carregando() || this.itens().length === 0) {
            this.toastError('Não há crachás disponíveis para impressão.');
            return;
        }
            ...
            .subscribe({
                next: async credenciais => {
                    try {
                        ...
                        console.error('Erro ao gerar crachás', erro);
                        this.toastError('Não foi possível gerar os crachás para impressão.');
                    }
                },
                error: erro => {
    ...
    private subtitulo(credencial: CredencialEvento): string {
        return credencial.tipo === 'SOBRINHO'
            ? 'Participante'
            : 'Equipe de Carona';
    }

    private descricaoSecundaria(credencial: CredencialEvento): string {
        if (credencial.tipo === 'SOBRINHO') {
            return credencial.responsavelNome
                ? `Responsável: ${credencial.responsavelNome}`
                : 'Participante do encontro';
        }

        return 'Acesso operacional do evento';
    }
